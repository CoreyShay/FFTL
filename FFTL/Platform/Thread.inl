...
#elif defined(__ORBIS__) && !FFTL_THREAD_USE_POSIX
...
{
#if defined(_MSC_VER)
	FFTL_VERIFY_EQ(0, ::WaitForSingleObject(h, INFINITE));
#elif defined(__ORBIS__) && !FFTL_THREAD_USE_POSIX
	FFTL_VERIFY_EQ(0, ::scePthreadJoin(h, nullptr));
#elif defined(_POSIX_VERSION)
...
inline void Thread::Start(ThreadOwner* pOwner, const char* pszName, enThreadPriority priority, uint coreAffinityMask, u32 stackSize)
{
	if (!GetIsRunning())
	{
		m_pOwner = pOwner;
