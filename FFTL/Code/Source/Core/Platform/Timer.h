...
class [[nodiscard]] TimerBase
{
public:
	TimerBase() = default;
	void Reset();
	void Accum();		// Increments the accumulation counter
	[[nodiscard]] u64 GetTicks() const	{ return m_TotalTicks; }
	[[nodiscard]] f64 GetAvgTicks() const { return m_TotalTicks / static_cast<f64>(m_nAccumCount); }
...
class [[nodiscard]] CpuTimer : public TimerBase
{
public:
	...
	[[nodiscard]] f64 GetMicroseconds() const				{ return ToMicroseconds(m_TotalTicks); }
	[[nodiscard]] f64 GetMilliseconds() const				{ return ToMilliseconds(m_TotalTicks); }
	[[nodiscard]] f64 GetSeconds() const					{ return ToSeconds(m_TotalTicks); }
	[[nodiscard]] f64 GetAvgMicroseconds() const			{ return ToMicroseconds(m_TotalTicks) / m_nAccumCount; }

	[[nodiscard]] static inline f64 ToMicroseconds(u64 t)	{ return t * sm_StaticInfo.m_TicksToUsScalar; }
	[[nodiscard]] static inline f64 ToMilliseconds(u64 t)	{ return t * sm_StaticInfo.m_TicksToUsScalar / 1000.0; }
	[[nodiscard]] static inline f64 ToSeconds(u64 t)		{ return t * sm_StaticInfo.m_TicksToUsScalar / 1000000.0; }

	[[nodiscard]] static const char* GetTickUnitsString();
...
class [[nodiscard]] Timer : public TimerBase
{
public:
	static u64 GetCurrentTicks();
