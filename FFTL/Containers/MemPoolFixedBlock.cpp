...
void FixedBlockMemPool::Init(void* pPool, uint maxCount, uint allocSize)
{
	FFTL_ASSERT(m_Pool == nullptr);
	FFTL_ASSERT(pPool);
	FFTL_ASSERT_MSG(maxCount <= ((u64)1 << (sizeof(_MaxCount)* 8)) - 1, "Integral type of _MaxCount must be made larger.");
	FFTL_ASSERT_MSG(allocSize <= ((u64)1 << (sizeof(_AllocSize)* 8)) - 1, "Integral type of _AllocSize must be made larger.");

	_MaxCount = static_cast<size_type>(maxCount);
...
void FixedBlockMemPool::Init(uint maxCount, uint allocSize, uint alignment)
{
